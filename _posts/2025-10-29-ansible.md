...
Michael was the maintainer of Cobbler, the provision service used by Satellite server. 

The term [ansible](https://en.wikipedia.org/wiki/Ansible) was first used by Ursula K. Le Guin in her 1966 novel Rocannon's World.
But Dehaan [said](https://groups.google.com/g/ansible-project/c/djZklLE2wIE/m/XCEOIq0RbN4J) he named the software after the fictional faster than light communication technology ansible in the novel Ender's Game.
...
## IaC
Infrastructure as Code (IaC) is the automation of infrastructure management and provisioning. This includes codifying the creation of VMs, the installation of softwares in the VM and how the services in it should work.
...
IaC can declarative or imperative. A declarative approach focuses on the outcome, the desired state. An imperative approach defines the step step procedures.
...
To a list of other IaC tools and the differences between, have a look at the table on this [Wikipedia page](https://en.wikipedia.org/wiki/Infrastructure_as_code). 
...
- It is a both declarative and imperative
...
- The managed nodes should be reachable by SSH while other method support are also supported.
- The inventory is list of managed nodes
...
- Modules is the code that are sent to the managed node and execute.
...
if you repeatedly run the command or playbook, Ansible knows that the program is already installed. It will not make any change. You see
`"changed": false` and `changed=0` in the respective methods.
...
It can be an IP address, FQDN or shortname as long the control node can resolve it.
...
- In the real world, the list of nodes will not be fixed. Ansible supports dynamic inventory for this situation. 
...
Pattern is how you select which nodes to target. It can be a host, the all group, your defined groups, wildcards or regex.
...
btw, `--become` a plugin is for the user to use sudo. 
...
Before for the playbook to correct format, the ansible-playbook error message said - "A playbook must be a _list_ of plays". According to the official doc, "A playbook consists of one or more ‘plays’ in an ordered _list_".

Lets focus on the word _list_. 

In the sample playbook, the top element is a list (sequence in YAML). The list has only one element whose content is two pairs of key/values (mapping in YAML). The first key/value is `hosts: all` which we use to select which nodes to target. The key for the second pair is `tasks` whose value will be a list again.
...
Lets take our bare bones playbook and run it through ansible linter ansible-lint.
...
```
   ...
   - shell: date
```
Note that we are not adding a new tasks. It is a part of the same tasks. It is a new key/value pair in the first tasks or list item.

### Violation 2
There's a module to run commands. By by running `date`, we are not using any shell functionality. Lets use the command module.
```
- name: Display the date on the remote host
  hosts: all
  tasks:
...
This should give us a solid understanding of the structure of a playbook. We can look at a 100 examples which we won't remember. But we can always construct a playbook if we know the basic structure. 
...
## Modules
Modules are the code that are sent to the remote host for execution. So, we have been using the builtin modules but one can write customer modules.
...
-  To the bottom (Shift +G) or search for EXAMPLE
- The options prefixed by = are mandatory. Example - `= name`, `= state` 

## Plugins
Plugins are sort of like modules but unlike modules, they extend the core functionality. We seen plugins like `become` for elevated privilege and `ansible_connection` to state type of connection to be used. 
...
- To override using environment varible
...
Example - Start httpd only if the install is succeeds. 
```
...
There are multiple ways to change the default setting. The following is the list of places to override the default in order of precedence, starting the lowest.
...
## Setup CICD
For my CICI Vagrant box, I had a shell scrip to install the packages and configure it. I had Gemini convert the shell script to playbook. It is an interesting observation to see the modules used to perform the equivalent function.  
